...
(function (global) {
  ...
  function usageLimitsKey(platform) {
    return `usage:${platform}`;
  }

  async function getUsageLimits(platform) {
    const key = usageLimitsKey(platform);
    const result = await chrome.storage.local.get(key);
    return result[key] || null;
  }

  async function saveUsageLimits(platform, limits) {
    const key = usageLimitsKey(platform);
    const record = { ...limits, updatedAt: Date.now() };
