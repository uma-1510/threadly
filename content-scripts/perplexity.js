...
(function () {
  ...
  function cleanTitle() {
    ...
    return raw.replace(/\s*[-–—|]\s*Perplexity\s*$/i, "").trim() || "Untitled conversation";
  }

  // Best-guess selectors, not verified against a live page -- Perplexity's
  // thread UI (query + answer, not a symmetric turn-by-turn chat) makes this
  // higher-risk than the ChatGPT/Claude selectors.
  function scanMessages() {
    const nodes = document.querySelectorAll(
      '[data-testid="query-text"], .prose'
    );
    const messages = [];
    nodes.forEach((node, idx) => {
      const role = node.matches('[data-testid="query-text"]') ? "user" : "assistant";
  ...
  async function syncConversation() {
    try {
      ...
      // A broken selector should stop capture, not break Perplexity for the user.
