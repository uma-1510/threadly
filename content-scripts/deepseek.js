...
(function () {
  ...
  function cleanTitle() {
    ...
    return raw.replace(/\s*[-–—|]\s*DeepSeek\s*$/i, "").trim() || "Untitled conversation";
  }

  // Best-guess selectors, not verified against a live page. DeepSeek's web UI
  // may use hashed/CSS-module class names generated per build, in which case
  // these will need to be rewritten entirely from real DOM, not just tweaked.
  ...
  async function syncConversation() {
    try {
      ...
      // A broken selector should stop capture, not break DeepSeek for the user.
